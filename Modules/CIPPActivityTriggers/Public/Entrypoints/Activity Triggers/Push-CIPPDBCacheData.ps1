function Push-CIPPDBCacheData {
    ...
    try {
        ...
        try {
            $DefenderCapable = Test-CIPPStandardLicense -StandardName Compliance'DefenderLicenseCheck' -TenantFilter $TenantFilter -RequiredCapabilities @('DEFENDER_FOR_ENDPOINT_PLAN_1', 'DEFENDER_FOR_ENDPOINT_PLAN_2', 'DEFENDER_FOR_BUSINESS') -SkipLog
        } catch {
            $ErrorMessage = Get-CippException -Exception $_
            Write-LogMessage -API 'CIPPDBCache' -tenant $TenantFilter -message "Compliance license check failed: $($_.Exception.Message)" -sev Warning -LogData $ErrorMessage
        }
        ...
        Write-LogMessage -API 'CIPPDBCache' -tenant $TenantFilter -message "License capabilities for $TenantFilter - Intune: $IntuneCapable, CA: $ConditionalAccessCapable, P2: $AzureADPremiumP2Capable, Exchange: $ExchangeCapable, Compliance: $ComplianceCapable" -sev Info
